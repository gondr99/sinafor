    ...
    <div class="role-component">
        <div>
            ...
            <div class="user-list p-2">
                <div class="user" :class="{active: selectUser !== null && selectUser.id === u.id}" v-for="u in managerList" @click="selectUser = u">{{u.name}}</div>
            </div>
        </div>
        <div>
...
<script>
    export default {
        ...
        methods:{
            addSkill(skillId){
                axios.put(`/admin/skill/manager/${skillId}`,{ user_id:this.selectUser.id }).then(res => {
                    this.$emit("refresh");
                });
            },
            removeSkill(skillId){
                axios.delete(`/admin/skill/manager/${skillId}?user_id=${this.selectUser.id}`).then(res => {
                    this.$emit("refresh");
                });
            },
            addAll(){
                axios.put(`/admin/skill/manager/all`,{ user_id:this.selectUser.id }).then(res => {
                    this.$emit("refresh");
                });
            },
            removeAll(){
                axios.delete(`/admin/skill/manager/all?user_id=${this.selectUser.id}`).then(res => {
                    this.$emit("refresh");
                });
            }
        },
        computed:{
            ownedSkillList(){
                ...
                return this.skillList.filter(s => s.managerList.find(u => u.id === this.selectUser.id) !== undefined);
            },
            otherSkillList(){
                if(this.selectUser === null) return [];
                return this.skillList.filter(s => s.managerList.find(u => u.id === this.selectUser.id) === undefined);
